/**
 * SSE (Server-Sent Events) utilities for Chat v2 (LangGraph backend).
 * Handles streaming responses from the orchestrator-langgraph service.
 */
...
/**
 * Connect to the Chat v2 (LangGraph) stream endpoint and process SSE events.
 *
 * @param request - Chat request parameters
 * @param callbacks - Event handlers for different SSE event types
 * @param signal - AbortSignal for cancellation
 */
export async function connectChatStreamV2(
  request: ChatRequest,
  callbacks: SSECallbacks,
  signal?: AbortSignal
): Promise<void> {
  // Use the v2 endpoint that connects to orchestrator-langgraph
  ...
  if (!response.ok) {
    ...
    throw new Error(`Chat V2 request failed: ${response.status} - ${errorText}`);
  }
      ...
      for (const eventText of events) {
        ...
        switch (event.type) {
          ...
          case "thinking":
            ...
            break;
          case "artifact":
            callbacks.onArtifact?.(event.data as ArtifactData);
            break;
          case "complete":
