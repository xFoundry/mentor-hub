...
const DEFAULT_PRE_BLOCK_CLASS =
  "my-4 w-full max-w-full overflow-x-auto rounded-xl bg-zinc-950 text-zinc-50 dark:bg-zinc-900 border border-border p-4";
...
const extractTextContent = (node: React.ReactNode): string => {
  ...
  if (isValidElement(node)) {
    // @ts-expect-error - children can be any node.
...
const components: Partial<Components> = {
  ...
  span: ({
    ...
    const dataLabel = props["data-label"] ?? props["data-name"];
  ...
  a: ({ children, ...props }: React.AnchorHTMLAttributes<HTMLAnchorElement>) => (
    <a
      className="font-medium underline underline-offset-4 whitespace-pre-wrap break-words"
      target="_blank"
      rel="noreferrer"
      {...props}
    >
  ...
  table: ({ children, ...props }: React.HTMLAttributes<HTMLTableElement>) => (
    <div className="my-6 w-full overflow-x-auto">
      <table className="relative w-full min-w-full border-none text-sm" {...props}>
        {children}
      </table>
    </div>
  ),
