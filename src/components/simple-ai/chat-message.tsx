  ...
  return (
    <Tooltip>
      <TooltipTrigger asChild>
        <span
          className={cn("text-xs text-muted-foreground cursor-default", className)}
          {...props}
        >
...
function ChatMessageContent({ className, ...props }: ComponentProps<"div">) {
  return (
    <div
      className={cn("w-full max-w-full flex flex-col gap-2 p-2 overflow-x-hidden", className)}
      {...props}
    />
  );
}
...
function ChatMessageAvatarFallback(props: ComponentProps<typeof AvatarFallback>) {
