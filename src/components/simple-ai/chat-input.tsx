  ...
  const addMentionConfig = useCallback((config: MentionConfig<any>) => {
    if (registeredTypesRef.current.has(config.type)) {
      setMentionConfigs((prev) => {
        const existingIndex = prev.findIndex((item) => item.type === config.type);
        if (existingIndex >= 0) {
          const updated = [...prev];
    ...
    >
      <InputGroup
        className={cn(
          ...
          "h-auto min-h-[3.25rem] items-stretch",
          className
        )}
        {...props}
      >
  ...
  return (
    <>
      ...
      <EditorContent
        editor={editor}
        className={cn(
          "w-full min-h-[3.25rem] max-h-60 px-4 pt-3 pb-2 overflow-y-auto overflow-x-hidden",
          className
        )}
      />
    </>
...
>({
  ...
  } as UseChatInputReturn<Mentions>;
}

type UnionToIntersection<U> = (U extends any ? (item: U) => void : never) extends (
  item: infer I
) => void
  ? I
  : never;
...
export function parseContent<Configs extends readonly MentionConfig<any>[]>(
  ...
  function recurse(node: JSONContent) {
    if (node.type === "text" && node.text) {
      ...
      const config = configs.find((item) => item.type === mentionType);
      if (config) {
